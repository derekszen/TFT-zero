...
def test_overnight_wrapper_writes_incremental_loop_artifacts(tmp_path) -> None:
    ...
    report = run_strategic_muzero_overnight(
        StrategicMuZeroOvernightConfig(
            ...
            codex_allowance_source="Codex usage dashboard",
            codex_five_hour_window_remaining="ample",
            codex_weekly_usage="ample",
    ...
    assert loop_state["codex_allowance_check"]["source"] == "Codex usage dashboard"
    assert metrics["config"]["codex_allowance_check"]["source"] == "Codex usage dashboard"
