...
def test_strategic_muzero_run_loop_runs_full_programmatic_gate(tmp_path) -> None:
    ...
    assert state["codex_allowance_check"]["weekly_usage"] == "waived"
    assert state["codex_allowance_check"]["decision"] == "continue"
    assert gate_state["codex_allowance_check"]["source"] == "user-waived"
    assert gate_state["codex_allowance_check"]["weekly_usage"] == "waived"
